...
| 分类        | 用途                                                                                 | 本地文档                    |
| ----------- | ------------------------------------------------------------------------------------ | --------------------------- |
| AI 模型接口 | 调用聊天、补全、嵌入、图像、音频、视频、重排序等 AI 能力，整体兼容 OpenAI API 风格。 | [AI 模型调用](./0.index.md) |
...
| 能力                            | 方法与路径                        | 说明                                         |
| ------------------------------- | --------------------------------- | -------------------------------------------- |
| [模型列表](./list.md)           | `GET /v1/models`                  | 获取当前可用模型列表。                       |
| [聊天](./completions.md)        | `POST /v1/chat/completions`       | 根据对话历史创建模型响应，支持流式和非流式。 |
| [补全](./completions-create.md) | `POST /v1/completions`            | 传统文本补全接口。                           |
| [嵌入](./embeddings-create.md)  | `POST /v1/embeddings`             | 将文本转换为向量嵌入。                       |
| [重排序](./rerank-create.md)    | `POST /v1/rerank`                 | 根据查询对文档列表做相关性重排序。           |
| [审查](./moderations-create.md) | `POST /v1/moderations`            | 对输入内容做安全审核。                       |
| [音频](./speech.md)             | `POST /v1/audio/speech` 等        | 文本转语音、音频转录、音频翻译等。           |
| [图像](./generations.md)        | `POST /v1/images/generations/` 等 | 图像生成、图像编辑等。                       |
| [实时语音](./session.md)        | `GET /v1/realtime`                | WebSocket 实时对话端点。                     |
| [视频](./sora-create.md)        | `POST /v1/videos` 等              | 视频生成任务、任务状态、视频内容获取。       |

详细参数和示例见 [AI 模型调用](./0.index.md)。
