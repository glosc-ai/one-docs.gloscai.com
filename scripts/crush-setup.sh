...
upsert_export() { local rcfile="${1:-}" key="${2:-}" value="${3:-}"; [ -n "$rcfile" ] || return 1; [ -n "$key" ] || return 1; mkdir -p "$(dirname "$rcfile")"; [ -f "$rcfile" ] || touch "$rcfile"; local line="export $key=$(sh_single_quote "$value")" pattern="^[[:space:]]*(export[[:space:]]+)?$key="; if grep -Eq "$pattern" "$rcfile"; then sed -i.bak -E "s|$pattern.*$|$line|g" "$rcfile"; rm -f "$rcfile.bak"; else printf '%s\n' "$line" >> "$rcfile"; fi; }
...
main() {
  local config_file="$HOME/.config/crush/crush.json" existing_token auth_token default_model
  existing_token="$(read_env_from_rcs 'ONE_API_KEY')"
  echo '=== Crush setup ==='; echo; echo "base_url will be set to: $DEFAULT_BASE_URL"; echo
