...
get_json_env_value() { local file_path="${1:-}" field_name="${2:-}"; [ -f "$file_path" ] || return 0; sed -n "s/.*\"$field_name\"[[:space:]]*:[[:space:]]*\"\([^\"]*\)\".*/\1/p" "$file_path" | head -n 1 | tr -d '\r\n'; }
prompt_required_value() {
  ...
  while true; do
    ...
    input_value="$(trim "$input_value")"; input_value="$(printf '%s' "$input_value" | tr -d '\r\n')"
    if [ -n "$input_value" ]; then printf '%s' "$input_value"; return 0; fi
    if [ -n "$existing_value" ]; then printf '%s' "$existing_value"; return 0; fi
    echo 'Value cannot be empty.' >&2; echo >&2
  done
}

main() {
  local config_file="$HOME/.deepcode/settings.json" existing_token existing_model auth_token default_model
  existing_token="$(get_json_env_value "$config_file" 'API_KEY')"
  existing_model="$(get_json_env_value "$config_file" 'MODEL')"
  echo '=== Deep Code setup ==='; echo; echo "BASE_URL will be set to: $DEFAULT_BASE_URL"; echo
