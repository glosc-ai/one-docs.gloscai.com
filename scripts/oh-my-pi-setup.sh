...
yaml_scalar() { printf '%s' "${1:-}" | sed "s/'/''/g; s/^/'/; s/$/'/"; }
...
main() {
  local config_file="$HOME/.omp/agent/models.yml" existing_token auth_token default_model escaped_model
  existing_token="$(read_env_from_rcs 'ONE_API_KEY')"
  echo '=== Oh My Pi setup ==='; echo; echo "baseUrl will be set to: $DEFAULT_BASE_URL"; echo
  ...
  escaped_model="$(yaml_scalar "$default_model")"
