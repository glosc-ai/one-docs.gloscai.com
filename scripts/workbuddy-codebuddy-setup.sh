...
main() {
  local config_file="$HOME/.codebuddy/models.json" chat_url existing_token auth_token default_model
  chat_url="${DEFAULT_BASE_URL%/}/chat/completions"
  existing_token="$(read_env_from_rcs 'ONE_API_KEY')"
  echo '=== WorkBuddy/CodeBuddy setup ==='; echo; echo "url will be set to: $chat_url"; echo
  ...
  cat > "$config_file" <<EOF
{
  ...
  "availableModels": ["$(json_escape "$default_model")"]
}
EOF
  ...
  echo 'Restart WorkBuddy/CodeBuddy after opening a new terminal session.'
}
